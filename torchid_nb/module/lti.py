...
from torchid_nb.functional.lti import MimoLinearDynamicalOperatorFun
...
class MimoLinearDynamicalOperator(torch.nn.Module):
    ...
    def forward(self, u_in, y_0=None, u_0=None):
        if self.n_k != 0:
            #u_d = u_in.roll(self.n_k, dims=-2)  # roll on the time axis # seems to be slow in PyTorch 1.4
